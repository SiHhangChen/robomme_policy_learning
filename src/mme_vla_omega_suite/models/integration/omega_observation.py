...
@at.typecheck
@struct.dataclass
class OmegaObservation(_Observation):
    ...
    # Omega memory is kept in its native cache layout until the model's shared
    # layer-fusion projection: [batch, history, view, layer, token, dim].
    omega_memory: at.Float[ArrayT, "b oh ov ol ot od"] | None = None
    omega_memory_mask: at.Bool[ArrayT, "b oh"] | None = None

    @classmethod
    def from_dict(cls, data: at.PyTree[ArrayT]) -> "OmegaObservation":
        ...
        return cls(
            ...
            symbolic_tokenized_prompt_mask=data.get("symbolic_tokenized_prompt_mask", None),
            omega_memory=data.get("omega_memory", None),
            omega_memory_mask=data.get("omega_memory_mask", None),
        )
    ...
    def to_dict(self) -> at.PyTree[ArrayT]:
        ...
        result["omega_memory"] = self.omega_memory
        result["omega_memory_mask"] = self.omega_memory_mask
        return result
    ...
    def from_base_obs(
        ...
        omega_memory: at.Float[ArrayT, "*b h v l t d"] | None = None,
        omega_memory_mask: at.Bool[ArrayT, "*b h"] | None = None,
    ) -> "OmegaObservation":
...
def preprocess_observation(
    ...
    **kwargs,
    ...
    return OmegaObservation.from_base_obs(
        base_obs,
        ...
        omega_memory=observation.omega_memory,
        omega_memory_mask=observation.omega_memory_mask,
    )
