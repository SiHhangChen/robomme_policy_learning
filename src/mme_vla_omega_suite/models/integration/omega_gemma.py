...
class MemoryRMSNorm(nn.Module):
    ...
    def __call__(self, x, cond=None):
        ...
        modulation = nn.Dense(x.shape[-1] * 2, kernel_init=kernel_init_out_proj, dtype=dtype)(
            cond
        )  # add small randomness instead of pure zeros
...
class MemoryAttention(nn.Module):
    ...
    def __call__(self, x, mem_seq, mem_mask):
        ...
        batch_size, mem_len, mem_width = mem_seq.shape
        _, x_len, x_width = x.shape
        num_heads = self.num_heads
        num_kv_heads = self.num_kv_heads
        head_dim = self.head_dim
        width = self.width
        assert mem_width == x_width == width
        if num_heads % num_kv_heads != 0 or num_heads * head_dim != width:
            raise ValueError("Invalid Omega memory attention head dimensions")
        ...
        q_positions = einops.repeat(jnp.arange(mem_len, x_len + mem_len), "t -> b t", b=batch_size)
        k_positions = einops.repeat(jnp.arange(mem_len), "t -> b t", b=batch_size)
        ...
        has_valid_memory = jnp.any(mem_mask, axis=-1)
        # A first decision can have no strict-past memory.  Let it attend a
        # zero dummy slot for numerical stability, then explicitly zero the
        # resulting residual so this branch has no effect.
        attn_mask = mem_mask[:, None, None, None, :]  # (B, 1, 1, 1, S)
        safe_attn_mask = jnp.logical_or(attn_mask, ~has_valid_memory[:, None, None, None, None])
        masked_logits = jnp.where(safe_attn_mask, logits, -2.3819763e38)
        ...
        encoded = out_einsum("BTNH,NHD->BTD", encoded)
        return jnp.where(has_valid_memory[:, None, None], encoded, jnp.zeros_like(encoded))
...
class HistoryBlock(nn.Module):
    ...
    def __call__(
        ...
        if self.integration_type == "modulation":
            ...
            if action_width % 4:
                raise ValueError("Action expert width must be divisible by four")
            mem_attn = MemoryAttention(
                width=action_width,
                num_heads=4,
                num_kv_heads=1,
                head_dim=action_width // 4,
                name="mem_attn",
            )
        ...
        for i, x_in in enumerate(xs):
            ...
            if x_in is not None:
                ...
                x, gate = RMSNorm(name=name)(x_in, adarms_cond[i])
            pre_attn.append(x)
        ...
        out = []
        gates = []
        for i, (x_in, config) in enumerate(zip(xs, self.configs, strict=True)):
            x = x_in
            if x_in is not None:
                ...
                x, gate = RMSNorm(name=name)(x, adarms_cond[i])
